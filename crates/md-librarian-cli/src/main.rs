//! `md-librarian` — the standalone book library viewer.
//!
//! Extracted from gpui-yaams (`yaams-books`) at v0.27.0-beta.3; the CLI is
//! unchanged apart from the names.
//!
//! Serves every book on the search path over a loopback origin and opens one
//! window on it. That is the whole binary: the interesting parts are
//! [`md_librarian`] (what counts as a book, and which copy wins) and
//! [`md_librarian_serve::shell`] (the page).
//!
//! ```text
